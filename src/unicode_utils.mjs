...
export function surrogatedCode(high, low) {
    return (high - 0xD800) * 0x400 + low - 0xDC00 + 0x10000;
}

export function getLengthAt(str, idx) {
    const first = str.charCodeAt(idx);
    if (!isHighSurrogate(first) || idx + 1 >= str.length)
        return [wcwidth(first), 1];
    const second = str.charCodeAt(idx + 1);
    if (!isLowSurrogate(second))
        return [wcwidth(first), 1];
    return [wcwidth(surrogatedCode(first, second)), 2];
}

export function getLengthBefore(str, idx) {
    const second = str.charCodeAt(idx - 1);
    if (!isLowSurrogate(second) || idx == 1)
        return [wcwidth(second), 1];
    const first = str.charCodeAt(idx - 2);
    if (!isHighSurrogate(first))
        return [wcwidth(second), 1];
    return [wcwidth(surrogatedCode(first, second)), 2];
}

export function getSubStringLength(str, start, end) {
    let res = 0;
    while (start < end) {
        const [disp_len, idx_len] = getLengthAt(str, start);
        start += idx_len;
        res += disp_len;
    }
    return res;
